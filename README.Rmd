...
`github_issue_authors()`, run in parallel across repos (each repo's calls
are independent of the others, so this is embarrassingly parallel).
`repo_url` is the natural unique identifier here (rather than `name`, which
is only unique within a single `source`), so results are keyed on it and
written alongside it, and `repo_tbl` is deduplicated on it before joining
so a repo appearing under multiple sources doesn't fan out the join.
