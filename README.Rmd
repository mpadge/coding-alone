...
columns directly). `repo_url` is the natural unique identifier here (rather
than `name`, which is only unique within a single `source`), so `repo_tbl`
is deduplicated on it before joining, so a repo appearing under multiple
