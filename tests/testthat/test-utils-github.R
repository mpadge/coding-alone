test_that ("normalize_github_url handles NA/empty input", {
    expect_true (is.na (longtail:::normalize_github_url (NA_character_)))
    expect_true (is.na (longtail:::normalize_github_url (NULL)))
    expect_true (is.na (longtail:::normalize_github_url ("")))
})

test_that ("normalize_github_url normalizes plain github.com URLs", {
    expect_equal (
        longtail:::normalize_github_url ("https://github.com/owner/repo"),
        "https://github.com/owner/repo"
    )
    expect_equal (
        longtail:::normalize_github_url ("http://github.com/owner/repo"),
        "https://github.com/owner/repo"
    )
    expect_equal (
        longtail:::normalize_github_url ("https://github.com/owner/repo.git"),
        "https://github.com/owner/repo"
    )
    expect_equal (
        longtail:::normalize_github_url ("https://github.com/owner/repo/issues"),
        "https://github.com/owner/repo"
    )
    expect_equal (
        longtail:::normalize_github_url ("git+https://github.com/owner/repo.git"),
        "https://github.com/owner/repo"
    )
})
...
test_that ("normalize_github_url handles the git@github.com: SSH form", {
    expect_equal (
        longtail:::normalize_github_url ("git@github.com:owner/repo.git"),
        "https://github.com/owner/repo"
    )
})
...
test_that ("normalize_github_url handles the github: shorthand", {
    expect_equal (
        longtail:::normalize_github_url ("github:owner/repo"),
        "https://github.com/owner/repo"
    )
})

test_that ("normalize_github_url returns NA for non-github URLs", {
    expect_true (is.na (longtail:::normalize_github_url ("https://example.com/owner/repo")))
    expect_true (is.na (longtail:::normalize_github_url ("not a url at all")))
})
...
test_that ("find_github_url returns first resolvable candidate", {
    ...
    expect_equal (longtail:::find_github_url (candidates), "https://github.com/owner/repo")
})
...
test_that ("find_github_url handles NULL entries and empty/all-non-matching input", {
    ...
    expect_true (is.na (longtail:::find_github_url (candidates)))
    expect_true (is.na (longtail:::find_github_url (character ())))
})

test_that ("parse_github_repo_url splits owner/repo", {
    parsed <- longtail:::parse_github_repo_url ("https://github.com/owner/repo")
    expect_equal (parsed, list (owner = "owner", repo = "repo"))

    parsed2 <- longtail:::parse_github_repo_url ("https://github.com/owner/repo.git")
    expect_equal (parsed2$repo, "repo")
})

test_that ("parse_github_repo_url errors on non-github URLs", {
    expect_error (
        longtail:::parse_github_repo_url ("https://example.com/owner/repo"),
...
test_that ("github_token reads GITHUB_TOKEN, falls back to GITHUB_PAT, else NA", {
    withr::with_envvar (
        ...
        expect_true (is.na (longtail:::github_token ()))
    )
    withr::with_envvar (
        c (GITHUB_TOKEN = "tok-1", GITHUB_PAT = "tok-2"),
        expect_equal (longtail:::github_token (), "tok-1")
    )
    withr::with_envvar (
        c (GITHUB_TOKEN = NA, GITHUB_PAT = "tok-2"),
        expect_equal (longtail:::github_token (), "tok-2")
    )
})
...
test_that ("github_respect_rate_limit is silent when quota is not nearly exhausted", {
    ...
    expect_silent (longtail:::github_respect_rate_limit (resp_ok))
})
...
test_that ("github_respect_rate_limit messages and waits when quota nearly exhausted", {
    ...
    expect_message (
        longtail:::github_respect_rate_limit (resp_low),
...
test_that ("github_respect_rate_limit errors if rate-limit headers are absent entirely", {
    ...
    expect_error (
        longtail:::github_respect_rate_limit (resp_missing),
